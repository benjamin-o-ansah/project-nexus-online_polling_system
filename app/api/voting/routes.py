...
# # ✅ allow these roles to vote when authenticated
...
#         403: {"description": "Forbidden (poll not active / role not allowed)"},
...
#     # ✅ allow SYSTEM_ADMIN to vote
#     allowed_roles = {"VOTER", "POLL_ADMIN", "SYSTEM_ADMIN"}
#     if user_id and role not in allowed_roles:
...
#             if user_id:
#                 # Authenticated (SYSTEM_ADMIN included) — no need for ANON_TOKEN_SECRET
...
#             else:
#                 # Anonymous flow — ✅ only now require the secret
...
#     except KeyError:
#         db.session.rollback()
#         current_app.logger.exception("Missing ANON_TOKEN_SECRET")
...
#         if user_id:
#             # ✅ SYSTEM_ADMIN works here automatically (it’s just another authenticated user_id)
