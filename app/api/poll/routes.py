...
@swag_from({
    ...
    "responses": {201: {"description": "Created"}, 400: {}, 403: {}}
})
def create_poll():
    payload = validate_or_abort(poll_create_schema, request.get_json(silent=True) or {})
    user_id = get_jwt_identity()
...
@swag_from({
    "tags": ["Polls"],
    "summary": "List polls",
    "description": "Admins see their own polls; others see active polls only.",
    "responses": {200: {}, 401: {}}
})
def list_polls():
    user_id = get_jwt_identity()
...
def get_poll(poll_id):
    ...
    if poll.status == Poll.STATUS_DRAFT:
        if not (role == "POLL_ADMIN" and str(poll.owner_id) == str(user_id)):
            secure_audit_log(
                "UNAUTHORIZED_POLL_VIEW_ATTEMPT",
                "POLL",
...
@swag_from({
    "tags": ["Polls"],
    "summary": "Update poll (draft only)",
...
def update_poll(poll_id):
    ...
    payload = validate_or_abort(poll_update_schema, request.get_json(silent=True) or {})
    ...
    with db.session.begin():
        ...
        if "description" in payload:
            poll.description = payload.get("description")
...
@swag_from({
    "tags": ["Polls"],
    "summary": "Publish poll (draft → active)",
    "responses": {200: {}, 400: {}, 403: {}, 404: {}}
})
def publish_poll(poll_id):
    poll = Poll.query.get(poll_id)
    ...
    try:
        with db.session.begin():
            poll.publish()
            audit_log("POLL_PUBLISHED", "POLL", entity_id=str(poll.id))
...
@swag_from({
    "tags": ["Polls"],
    "summary": "Close poll (active → closed)",
    "responses": {200: {}, 400: {}, 403: {}, 404: {}}
})
def close_poll(poll_id):
    poll = Poll.query.get(poll_id)
    ...
    try:
        with db.session.begin():
            poll.close()
            audit_log("POLL_CLOSED", "POLL", entity_id=str(poll.id))
...
def delete_poll(poll_id):
    ...
    with db.session.begin():
        audit_log("POLL_DELETED", "POLL", entity_id=str(poll.id))
        db.session.delete(poll)

    return {"message": "Poll deleted successfully"}, 204